...
"""
测试结果可视化脚本
读取method_comparison_results.csv文件，生成各种性能指标的比较图表
"""
...
class ResultsVisualizer:
    ...
    def plot_comprehensive_radar_chart(self):
        ...
        # Select key metrics for radar chart comparison
        ...
        # For "lower is better" metrics, use 1-normalized_value
        reverse_metrics = ['variance', 'cv', 'dead_nodes', 'low_energy_nodes']
        for metric in metrics:
            if metric in reverse_metrics:
                # For lower is better metrics, normalize then reverse
                ...
                # For higher is better metrics, normalize directly
                min_val = normalized_data[metric].min()
                max_val = normalized_data[metric].max()
                if max_val > min_val:
        ...
        ax.set_ylim(0, 1)
        ax.set_title('Comprehensive Performance Radar Chart\n(Values closer to outer ring indicate better performance)', 
    ...
    def plot_performance_ranking(self):
        ...
        # Calculate ranking of each method on different metrics
        ranking_metrics = {
    ...
    def plot_method_comparison_summary(self):
        ...
        for idx, (metric, title, note) in enumerate(metrics_info):
            ...
            # Ensure best_idx is within valid range
            if 0 <= best_idx < len(bars):
                bars[best_idx].set_color('#FFD700')  # Gold highlight for best performance
...
def main():
    """主函数"""
    # CSV文件路径
    csv_file = "data/method_comparison_results.csv"
    
    # 检查文件是否存在
    if not os.path.exists(csv_file):
        print(f"错误: 找不到结果文件 {csv_file}")
        print("请先运行 test_methods.py 生成测试结果")
        return
    
    # 创建可视化器并生成所有图表
    visualizer = ResultsVisualizer(csv_file, output_dir="testRe")
