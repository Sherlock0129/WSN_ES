...
class SimulationConfig:
    ...
    check_interval: int = 20                # 智能检查间隔（分钟）
...
class SchedulerConfig:
    ...
    cluster_round_period: int = 720          # 轮换簇首周期（分钟）
    ...
    ddpg_save_interval: int = 10             # 模型保存间隔（每N回合保存一次）
    ddpg_action_dim: int = 1                 # DDPG动作维度（1=传输时长）
    ddpg_action_min: float = 1.0             # 动作最小值（分钟），让DDPG自己探索
    ddpg_action_max: float = 10.0            # 动作最大值（分钟），扩大探索空间
...
class ADCRConfig:
    ...
    # 核心算法参数
    round_period: int = 720        # 重聚类周期（分钟），例如每日重选簇头
...
class EETORConfig:
    ...
    # 能量状态感知参数
    enable_energy_state_aware: bool = False  # 是否启用能量状态感知
...
class ConfigManager:
    ...
    def get_scheduler_params(self) -> Dict[str, Any]:
        ...
        elif scheduler_type == "ThresholdScheduler":
            return {
                "K": self.scheduler_config.lyapunov_k,
                "max_hops": self.network_config.max_hops
            }
