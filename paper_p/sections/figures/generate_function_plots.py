...
# 参数设置
A_max_0 = 500  # 基准AOI上限（分钟）
gamma = 6000000   # 刻度因子
I = np.linspace(0, 10000000, 10000000)  # 信息量范围
...
plt.plot(I, A_max, 'b-', linewidth=2.5, label='Dynamic AOI Upper Bound $A_{\\max,i}(t)$')

# 标注几个关键点
key_points = [0, 1000000, 3000000, 5000000, 10000000]
...
plt.xlabel('Information Volume $I_i(t)$', fontsize=13, fontweight='bold')
plt.ylabel('Dynamic AOI Upper Bound $A_{\\max,i}(t)$ (min)', fontsize=13, fontweight='bold')
plt.title('Illustration of Dynamic AOI Upper Bound Adaptation', fontsize=14, fontweight='bold')
...
print(f"  ✓ Saved: {output_dir}aoei_adaptive_threshold.png")
plt.close()
...
# def apply_jitter(data, amp):
#     noise = amp * rng.standard_normal(size=data.shape)
#     return data + noise

# # 为每个指标引入不同幅度的轻微震荡，使趋势更贴近“有噪环境”
# B_energy_plot = apply_jitter(B_energy, 0.02 * B_energy.max())
# P_loss_plot = apply_jitter(P_loss, 0.03 * P_loss.max())
# P_aoi_plot = apply_jitter(P_aoi, 0.015 * P_aoi.max())
# R_info_plot = apply_jitter(R_info, 0.025 * R_info.max())

# Score = B_energy_plot - P_loss_plot - P_aoi_plot + R_info_plot

# # 创建图形
# plt.figure(figsize=(10, 7))

# # 绘制各项指标
# plt.plot(tau, B_energy_plot, 'g-', linewidth=2.5, label='能量收益 $B_{\\text{energy}}(\\tau)$')
# plt.plot(tau, -P_loss_plot, 'r--', linewidth=2, label='损耗惩罚 $-P_{\\text{loss}}(\\tau)$')
# plt.plot(tau, -P_aoi_plot, 'm--', linewidth=2, label='时效惩罚 $-P_{\\text{aoi}}(\\tau)$')
# plt.plot(tau, R_info_plot, 'b--', linewidth=2, label='信息奖励 $R_{\\text{info}}(\\tau)$')
