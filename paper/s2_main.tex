...
\textbf{节点建模（对应实现：\texttt{src/core/SensorNode.py}）：}网络由普通传感器节点与物理中心节点组成。物理中心节点（\texttt{ID=0}）不参与能量传输，其职责在于信息汇聚与全网状态维护（\texttt{NodeInfoManager}）。普通节点\(i \in \mathcal{N}\)具备以下关键属性与动态过程：
\begin{itemize}
    \item \emph{能量状态}：电池容量\(C_i\)（典型值3.5~mAh, 3.7~V，对应\texttt{capacity}与\texttt{voltage}参数），当前能量\(E_i(t)\)（\texttt{current\_energy}），低/高阈值\(\theta_i^{\text{low}}, \theta_i^{\text{high}}\)（\texttt{low\_threshold\_energy}, \texttt{high\_threshold\_energy}）。
    \item \emph{能量采集}（\texttt{energy\_harvest(t)}）：若启用太阳能（\texttt{has\_solar=True}），基于日内辐照度模型\(G(t) = G_{\max} \sin(\pi(t-t_{\text{rise}})/(t_{\text{set}}-t_{\text{rise}}))\)采集能量，转换效率\(\eta_{\text{solar}}=0.2\)，面板面积\(A=0.1~\text{m}^2\)。
    \item \emph{能量消耗}（\texttt{energy\_consumption(target, transfer\_WET)}）：感知能耗\(E_{\text{sen}}=0.1~\text{J}\)，通信能耗\(E_{\text{com}} = E_{\text{elec}} B + \epsilon_{\text{amp}} B d^{\tau}\)（其中\(B\)为比特率，\(d\)为距离，\(\tau=2\)为路径损耗指数）。
    \item \emph{能量传输效率}（\texttt{energy\_transfer\_efficiency(target)}）：距离\(d\)处的无线能量传输效率\(\eta(d) = \eta_0 / d^{\gamma}\)（\(\eta_0=0.6\), \(\gamma=2.0\)，对应1米处最大效率60\%）。
\end{itemize}
每个物理节点在信息空间一一映射为轻量级代理\texttt{InfoNode}（\texttt{src/info\_collection/info\_node.py}），用于调度器与路由算法访问，避免"上帝视角"。
...
\subsection{Economic Interpretation and AOI as Price Signal}
经济学解释围绕"价格信号—数字账户—交易规则"的结构展开。

\textbf{（1）AOI 作为价格信号（实现：\texttt{NodeInfoManager.latest\_info['aoi']}）：}信息年龄\(\text{AOI}_i(t)\)定义为信息到达物理中心后的时间：
\begin{equation}
\text{AOI}_i(t) = t - t_{\text{arrival},i}
\end{equation}
其经济学含义类似于\emph{资产折旧}或\emph{商品保质期}：节点\(i\)的能量状态信息随时间"过时"，决策质量下降。当\(\text{AOI}_i(t)\)较大时，表明该节点的能量信息陈旧，可能处于低能状态却未及时触发传能，导致"逆向选择"；因此，应提高价格信号（优先级）以吸引供能节点。信息价值的时间衰减可建模为：
\begin{equation}
V_{\text{info},i}(t) = V_0 \cdot e^{-\lambda \cdot \text{AOI}_i(t)}
\end{equation}
其中\(\lambda\)为衰减率（类似金融学的折现率），\(V_0\)为新鲜状态下的基础价值。在决策函数中，AOI以惩罚项形式内生化：
\begin{equation}
C_{\text{AOI},i}(\tau) = w_{\text{aoi}} \cdot \tau \cdot Q_i
\end{equation}
其中\(\tau\)为传输时长（导致AOI增量），\(Q_i\)为节点\(i\)的能量虚拟队列长度（需求紧迫度），\(w_{\text{aoi}}\)为权重系数（默认0.1）。

\textbf{（2）InfoNode 数字账户（实现：\texttt{src/info\_collection/physical\_center.py:36-96}）：}维护三级缓存架构：
\begin{itemize}
    \item \emph{L1-最新状态表}：\texttt{latest\_info[node\_id] = \{energy, aoi, position, is\_solar, ...\}}，支持\(O(1)\)查询。
    \item \emph{L2-近期历史}：固定大小1000的FIFO队列，用于趋势分析与异常检测。
    \item \emph{L3-长期归档}：CSV批量写入，支持离线分析与模型训练。
\end{itemize}
该架构提供"透明市场视图"，弱化信息不对称，避免道德风险。

\textbf{（3）机会主义信息上报（实现：\texttt{PathBasedInfoCollector}）：}在传能路径上"搭载"状态更新，结合信息去重（基于源节点集合）与动态等待上限\(T_{\max}/(1+\text{info\_volume}/\text{scale})\)，在通信开销与信息新鲜度之间实现自适应权衡。

上述三者相互耦合：价格信号决定分配优先级与强度，数字账户保障状态可得，交易规则降低获取状态的边际成本；辅以ADCR路由的效率阈值与多跳抑制，可在局部理性前提下导出全局可解释的资源再配置结果。

\subsubsection{博弈论视角：能量共享的合作博弈与机制设计}

本文将多节点能量共享建模为\emph{合作博弈}，每个节点在"保守能量"与"参与共享"之间进行策略选择。

\textbf{博弈模型：}设局中人集合为\(\mathcal{N} = \{1,2,\ldots,N\}\)。节点\(i\)的策略空间包括：（i）作为需求方，通过AOI信号表达"支付意愿"\(p_i(t) = f(\text{AOI}_i, E_i, \theta_i)\)；（ii）作为供给方，决定是否响应传能请求及传输时长\(\tau_i \in [0, \tau_{\max}]\)。收益函数为：
\begin{equation}
u_i(E_i, E_{-i}, \tau) = \alpha_1 E_{\text{received},i} - \alpha_2 E_{\text{sent},i} - \alpha_3 \text{AOI}_i \cdot \tau
\end{equation}
其中\(E_{\text{received},i}\)为节点\(i\)接收的净能量，\(E_{\text{sent},i}\)为作为供能方发送的能量（含损耗），\(\text{AOI}_i \cdot \tau\)为传能时长导致的信息过时成本。

\textbf{准纳什均衡：}在AOI价格信号与弱势保护机制下，系统存在\emph{准纳什均衡}（Quasi-Nash Equilibrium），满足：（i）\emph{个体理性}（Individual Rationality）——每个节点的收益不低于不参与合作的收益；（ii）\emph{预算平衡}（Budget Balance）——系统总能量守恒；（iii）\emph{激励相容}（Incentive Compatibility）——节点通过InfoNode如实报告能量状态是其最优策略（因信息透明化消除了隐藏信息的收益）。

\textbf{帕累托改进与边界外移：}传统静态均衡下，系统位于固定的帕累托前沿\(\mathcal{F}_{\text{static}}\)。本文通过三类机制性改造实现\emph{动态帕累托边界外移}：（a）价格内生化——将外生的"谁需要能量"转化为内生的市场定价（AOI驱动），减少信息不对称；（b）信息透明化——InfoNode数字孪生降低逆向选择与道德风险；（c）外部性治理——ADCR效率阈值与跳数限制抑制低效多跳的负外部性。形式化地，可达解集满足\(\mathcal{F}_{\text{dynamic}} \supset \mathcal{F}_{\text{static}}\)，即在相同资源约束下，存在\((\eta, \phi) \in \mathcal{F}_{\text{dynamic}}\)使得效率\(\eta\)与公平度\(\phi\)同时优于静态基线。该主张的实证验证见第五章。
...
\subsection{E1：信息价值定价缺失 \texorpdfstring{$\rightarrow$}{→} T1：触发/排序/预算清算 \texorpdfstring{$\rightarrow$}{→} M1：AOI定价与智能触发}
\textbf{经济学问题（E1）}：缺乏可解释的价格信号会导致传能时机与对象选择失准，出现"该救的不救、该等的不等"的资源错配与市场失灵。

\textbf{技术问题（T1）}：如何将信息新鲜度、节点能量紧迫性与系统均衡需求内生化为触发与排序的价格化规则？

\textbf{机制（M1）}（实现：\texttt{src/scheduling/passive\_transfer.py:PassiveTransferManager}）：采用\emph{智能被动传能触发机制}，综合以下决策因素：
\begin{enumerate}
...
\textit{完整实现}: \texttt{passive\_transfer.py:57-246}，其中第82-153行为核心决策逻辑。
...
\textbf{机制（M4）}（实现：\texttt{src/dynamic\_k/lookahead.py:pick\_k\_via\_lookahead}）：基于\emph{前瞻性仿真}的动态\(K\)值优化策略：
\begin{enumerate}
...
\textit{完整实现}: \texttt{lookahead.py:72-97}，配合\texttt{\_eval\_one\_candidate()}函数（第16-70行）。该机制可与Lyapunov调度器、DQN/DDPG等方法无缝集成。
...
本节给出仿真的详细参数配置，所有参数均可在配置文件\texttt{config\_examples/adaptive\_duration\_aware\_lyapunov\_config.py}中查阅与调整。
...
& 通信半径 & 30 m \\
\hline
\multicolumn{3}{l}{\emph{能量参数（\texttt{SensorNode}）}} \\
& 初始能量 & 20000 J \\
& 电池容量 & 3.5 mAh \\
& 电压 & 3.7 V \\
& 低能量阈值 & 30\% (6,664 J) \\
...
\hline
\multicolumn{3}{l}{\emph{被动传能（\texttt{PassiveTransferManager}）}} \\
& 检查间隔 \(T_{\text{check}}\) & 10 分钟 \\
& 临界比例 \(r_{\text{crit}}\) & 0.2 \\
& 能量方差阈值 \(\text{CV}_{\text{th}}\) & 0.3 \\
& 冷却期 \(T_{\text{cool}}\) & 30 分钟 \\
\hline
\multicolumn{3}{l}{\emph{动态K值（\texttt{lookahead}）}} \\
& 前瞻窗口 \(T_h\) & 60 分钟 \\
& 滞回阈值 \(h\) & 0.05 \\
& \(K_{\max}\) & 5 \\
\hline
\multicolumn{3}{l}{\emph{AOI与调度（\texttt{DurationAwareLyapunovScheduler}）}} \\
& AOI权重 \(w_{\text{aoi}}\) & 0.1 \\
& 信息量权重 \(w_{\text{info}}\) & 0.05 \\
& Lyapunov参数 \(V\) & 100.0 \\
\hline
\multicolumn{3}{l}{\emph{ADCR路由}} \\
& 最大跳数 \(H_{\max}\) & 5 \\
...
\textbf{路由与调度：}被动传能触发以AOI与能量紧迫度主导；ADCR路由限定最大跳数5跳，效率阈值0.05；前瞻K值优化窗口60分钟。
...
\hline
无能量共享 & 仅依赖太阳能采集与自然消耗，下界参考 & \texttt{enable\_sharing=False} \\
\hline
Lyapunov & 虚拟队列优化，最小化能量方差：\(Q_i(t+1) = [Q_i + \theta_i - E_i]^+\)，传能决策：\(\max \sum Q_j \Delta E_j\) & \texttt{LyapunovScheduler} \\
\hline
DurationAware & Lyapunov + 时长成本惩罚：\(C = Q_j E_j - V \cdot \text{loss} - w_{\tau} \tau\) & \texttt{DurationAwareLyapunovScheduler} \\
\hline
DQN & 离散动作空间：\(a \in \{0,1,\ldots,K_{\max}\}\)；状态：\((E, \text{CV}, \text{AOI})\)；奖励：\(r = -\text{CV} + 0.1 E_{\text{eff}}\)；训练1000轮，\(\epsilon\)-greedy=0.1 & PyTorch实现 \\
\hline
DDPG & 连续动作空间：\(a \in [0, \tau_{\max}]\)；Actor-Critic架构；奖励同DQN；Ornstein-Uhlenbeck噪声；训练1000轮 & PyTorch实现 \\
\hline
\textbf{本文方法} & Lyapunov + AOI + InfoNode + PassiveTransfer + ADCR + lookahead + 弱势保护 & \textbf{全模块集成} \\
\hline
\end{tabular}
\end{table}

\textbf{公平性保障：}所有方法使用相同的能量模型（传输效率、消耗、采集）、拓扑配置（节点位置、初始能量）、评估指标（寿命、CV、效率、AOI）及随机种子（10次重复）。深度学习基线（DQN/DDPG）的训练与测试协议统一：训练集为前5000分钟，测试集为后5080分钟；超参数遵循各方法的标准设置\cite{drlSurvey}。

\subsection{Ablation Studies：消融与变体}
为洞察各机制贡献，设计如下消融实验：
\begin{itemize}
    \item \textbf{去除AOI价格信号}：以固定或静态阈值替代价格化触发（即不使用\texttt{PassiveTransferManager}的AOI相关逻辑），考察触发准确度与效率变化；
    \item \textbf{去除InfoNode/机会主义上报}：改为周期上报（如每30分钟）或无上报，考察信息新鲜度与通信开销变化；
    \item \textbf{去除去重/动态等待}：禁用信息去重与动态等待上限（\texttt{PathBasedInfoCollector}中的相关逻辑），计算信息冗余与上报负载对性能的影响；
    \item \textbf{去除ADCR路由约束}（无效率阈值/最大跳数）：允许任意低效路径，观察低效多跳比例与系统能效变化；
    \item \textbf{固定K值}：替换前瞻性K值优化（\texttt{lookahead}）为固定值\(K=3\)，对比单位能量收益与自适应性能；
