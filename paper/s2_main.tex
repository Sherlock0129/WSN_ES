...
\textbf{节点建模：}网络由普通传感器节点与物理中心节点组成。物理中心节点（\texttt{ID=0}）不参与能量传输，其职责在于信息汇聚与全网状态维护（\texttt{NodeInfoManager}）。普通节点\(i \in \mathcal{N}\)具备以下关键属性与动态过程：
\begin{itemize}
...
\end{itemize}
每个物理节点在信息空间一一映射为轻量级代理\texttt{InfoNode}，用于调度器与路由算法访问，避免"上帝视角"。
...
\textbf{（1）AOI 作为价格信号：}信息年龄\(\text{AOI}_i(t)\)定义为信息到达物理中心后的时间：
\begin{equation}
...
\end{equation}
其中\(\lambda\)为衰减率（类似金融学的折现率），\(V_0\)为新鲜状态下的基础价值。在需要时，信息量可作为放大因子或加法收益项并入“信息价值”指标。在决策函数中，AOI以惩罚项形式内生化：
\begin{equation}
...
\textbf{帕累托改进与边界外移：}传统静态均衡下，系统位于固定的帕累托前沿\(\mathcal{F}_{\text{static}}\)。本文通过三类机制性改造实现\emph{动态帕累托边界外移}：（a）价格内生化——将外生的"谁需要能量"转化为内生的市场定价（AOI驱动），减少信息不对称；（b）信息透明化——InfoNode数字孪生降低逆向选择与道德风险；（c）外部性治理——能量传输专用路由的效率阈值与跳数限制抑制低效多跳的负外部性。形式化地，可达解集满足\(\mathcal{F}_{\text{dynamic}} \supset \mathcal{F}_{\text{static}}\)，即在相同资源约束下，存在\((\eta, \phi) \in \mathcal{F}_{\text{dynamic}}\)使得效率\(\eta\)与公平度\(\phi\)同时优于静态基线。该主张的实证验证见第五章。
...
\textbf{路由与调度：}被动传能触发以AOI与能量紧迫度主导；能量传输专用路由限定最大跳数5跳、效率阈值0.05；前瞻K值优化窗口60分钟。
...
\begin{itemize}
    ...
    \item \textbf{信息新鲜度}（AoEI/AoI相关）：越新鲜越好（越低龄越好）；必要时报告\emph{信息价值}（AOEI×信息量）作为参考；
...
\hline
\textbf{本文方法} & Lyapunov + AOI + InfoNode + PassiveTransfer + EETOR + lookahead + 弱势保护 & \textbf{全模块集成} \\
\hline
\end{tabular}
\end{table}
