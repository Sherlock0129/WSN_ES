...
\parbox[c][4.0cm][c]{0.95\linewidth}{\centering \textbf{图占位：网络拓扑与能量路径示意图}\\[3pt]
建议后续替换为仿真生成的节点分布与路径示意图。}
\end{mdframed}
\caption{网络拓扑与能量路径示意（占位）。}
...
\begin{mdframed}[linewidth=0.5pt]
\centering
\parbox[c][4.0cm][c]{0.95\linewidth}{\centering \textbf{图占位：AOEI 导致信息价值指数衰减曲线}\\[3pt]
建议以代码生成并替换此处曲线图（可扩展自 \texttt{src/experiments/visualize\_results.py}）。}
\end{mdframed}
...
% 图占位：动态帕累托边界外移示意（保留位置，后续替换）
\begin{figure}[t]
\centering
\begin{mdframed}[linewidth=0.5pt]
\centering
\parbox[c][4.0cm][c]{0.95\linewidth}{\centering \textbf{图占位：动态帕累托边界外移示意图}\\[3pt]
建议替换为实证对比图（如综合雷达图/效率-公平二维对标）。}
\end{mdframed}
\caption{动态帕累托边界外移的概念示意。}
\label{fig:dynamic_pareto_placeholder}
\end{figure}
