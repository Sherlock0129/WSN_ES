...
\begin{itemize}
    ...
    \item \textbf{M2（InfoNode数字孪生机制）：}验证理论能量计算维护虚拟节点能量表的准确性，以及信息价值加权对通信成本与调度准确率的影响。
...
\midrule
\multicolumn{3}{@{}l}{\textit{M2：InfoNode数字孪生机制参数}} \\
...
\begin{itemize}
    ...
    \item \textbf{E2（M2 InfoNode数字孪生机制）：}验证理论能量计算维护虚拟节点能量表的准确性，以及信息价值加权对通信成本与调度准确率的影响。对比方法：无InfoNode缓存、无信息价值奖励。
...
\begin{itemize}
    ...
    \item \textbf{无InfoNode（No-InfoNode）：}取消数字孪生缓存，采用周期上报获取节点状态。
    \item \textbf{无信息价值奖励（No-InfoReward）：}保留InfoNode缓存，但不进行信息价值加权。
...
\begin{itemize}
    ...
    \item \textbf{Ablation-M2-1：}去除InfoNode理论能量计算 \(\Rightarrow\) 观察调度准确率和信息滞后导致的错误调度事件数。
...
上述流程与第 4 章的机制设计一致：AOEI优先级机制 + InfoNode数字孪生机制 + ALDP自适应时长规划机制 + EETOR机会主义上报机制，能够在不增加总能量消耗的前提下，同时实现网络寿命的显著延长和能量分布的充分平衡。
...
\paragraph{E2：InfoNode数字孪生机制验证（M2）}

E2实验验证了InfoNode数字孪生机制在维护虚拟节点能量表和降低通信开销方面的有效性。

\textbf{理论能量计算的效果：}InfoNode机制通过理论能量计算维护虚拟节点能量表，在节点的真实信息没有上报到sink节点之前，能够基于能量衰减模型、太阳能采集模型等物理规律估算节点当前能量状态。实验结果表明，理论能量估算的误差率平均仅为3.2\%，显著低于无InfoNode缓存情况下的信息滞后导致的调度误差（平均误差率15.8\%）。
...
\textbf{通信开销降低：}通过对比无InfoNode缓存（采用周期上报）和InfoNode机制（利用能量传输路径同步状态），发现InfoNode机制将通信开销从97.84 kJ降低至21.01 kJ，降幅达78.5\%，验证了InfoNode机制通过利用能量传输路径同步状态，避免独立通信开销的有效性。
...
\begin{figure}[t]
  ...
  \caption{E2：不同信息层配置下的 AOEI 分布（越左越新鲜）。InfoNode+信息价值加权显著改善了信息新鲜度。}
...
\begin{itemize}
  ...
  \item \textbf{通信开销：}提案机制通过InfoNode数字孪生和EETOR机会主义上报，将通信开销从97.84 kJ降低至21.01 kJ，降幅达78.5\%，验证了信息与能量双生系统在降低通信开销方面的有效性。
\end{itemize}

总体而言，"AOEI优先级机制 + InfoNode数字孪生机制 + ALDP自适应时长规划机制 + EETOR机会主义上报机制"在既定能量/通信预算下，同时实现了网络寿命的显著延长和能量分布的充分平衡，印证第~\ref{sec:mech_design} 章的机制协同与核心目标。四个机制相互耦合、协同工作，共同实现了信息与能量双生系统，从根本上整合了能量与信息系统，解决了"信息系统独立于能量系统"这一核心问题。
...
\begin{table}[t]
  \centering
  \caption{Experiment~2（E2）：InfoNode数字孪生机制对能效与通信代价的影响}
  \label{tab:e2_info}
  ...
  \begin{tabular}{l S[table-format=2.2] S[table-format=4.2] S[table-format=2.2] S[table-format=1.2] S[table-format=4.0] S[table-format=1.2]}
    ...
    \midrule
    InfoNode+价值加权 & 51.90 & 4161.06 & 21.01 & 0.68 & 504 & 99.3 \\
...
未来的优化方向包括：（1）进一步优化动态AOI上限的计算策略，提高能量信息上报时机的准确性；（2）增强InfoNode理论能量计算的精度，减少估算误差；（3）探索更复杂的自适应时长规划策略，进一步提升能量-信息协同优化效果；（4）研究EETOR机制在不同网络拓扑下的适应性与鲁棒性。此外，该机制在不同环境下的适应性与鲁棒性也是未来研究的重要方向。
