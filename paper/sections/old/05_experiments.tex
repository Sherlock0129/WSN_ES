...
为控制实验的统计误差，对所有配置进行 10 次独立的随机种子重复，并报告均值及其 95\% 的置信区间（Bootstrap 方法，1000 次重采样）。
...
\begin{tabular}{@{}llp{4cm}@{}}
  ...
  \multicolumn{3}{@{}l}{\textit{被动传能触发策略}}                               \\
                & 检查间隔          & 每 1 分钟评估全网能量分布                         \\
                & 低能比例阈值        & 低能节点占比超过 0.05 时触发                      \\
                & 方差阈值          & 能量方差超过 0.05 时触发调节                      \\
...
为与第四章中的机制要素一一对应，实验以 M1--M4 为主线设计四类主实验（E1--E4），并辅以系统化对照与消融。
...
\paragraph{对照方法（Comparators）}
本文将“Baseline”明确定义为“本方法的完整配置”（价格触发 + 信息透明层/机会主义上报 + 路径治理 + 自适应时长）。所有对比均采用“组件替换”的方式，将Baseline中的某一机制替换为他法/禁用项形成对照，以检验该机制的边际贡献与制度权衡。为保证可复现和公平对比，统一物理/统计协议下设置如下对照方法：
\begin{itemize}
...
\begin{enumerate}
  ...
  \item 用 10 次独立种子下的 Bootstrap（1000 次）构造 95\% 置信区间，并以配对 Wilcoxon 检验比较“提案 vs. 各对照”的 HV 与关键指标（寿命、CV、效率）。
...
% Baseline energy trajectory figure (DurationAware + Opportunistic)
\begin{figure}[t]
  \centering
  \includegraphics[width=0.92\linewidth]{figures/e0_baseline.png}
  \caption{Baseline（DurationAware + 机会主义上报）：节点能量随时间的演化，用于与不同制度/策略进行对比。}
  \label{fig:e0_baseline}
\end{figure}

\paragraph{通信上报制度（E3）}
本节仅比较两种制度：机会主义上报（本方法的信息透明层）与 ADCR。最新结果显示二者在通信代价、触发频率、体验反馈与弱势覆盖上呈现清晰的制度权衡。

首先，从通信代价与触发频率看（图\ref{fig:e3_comm_costs_reports}）：机会主义上报的信息通信能耗仅为 21.01 kJ，而 ADCR 达到 140.95 kJ，后者高出约 6.7 倍；在上报/传能频次上，ADCR 为 613 次，略高于机会主义的 504 次，表明 ADCR 的制度会更频繁地驱动网络动作并带来更高的通信/协调开销。

其次，从体验与公平维度看（图\ref{fig:e3_feedback_fairness}）：机会主义上报的平均反馈分数为 3.80，显著高于 ADCR 的 0.30，表明在既定资源约束下，机会主义策略能以更少的通信行动带来更高的总体满意度与调度效果；同时，ADCR 在弱势节点覆盖率上达到 100\%，优于机会主义的 83\%，体现其对低能节点的“强覆盖”偏好，但这一公平性收益伴随了明显的通信能耗与频次攀升。

最后，能量轨迹（图\ref{fig:e3_adcr_energy_traj}）揭示了 ADCR 在全时域上的能量演化形态：多段“快速回落—短时回补”的锯齿形波动较为突出，后期整体能量下沉更明显。这与其更高的触发频率与通信/协调负担一致，提示 ADCR 更倾向以高频干预换取覆盖性；而机会主义上报在不显著增加触发频次的前提下，通过价值加权与信息缓存抑制了无效上报与过度干预，从而以更低成本支撑更高的主观反馈分数。

% E3 figures: comm costs & reports, feedback fairness
\begin{figure}[t]
  \centering
  \includegraphics[width=0.92\linewidth]{figures/e3_comm_costs_reports.png}
  \caption{E3：通信成本与上报/传能频次（仅比较机会主义 vs ADCR）。机会主义：21.01 kJ, 504 次；ADCR：140.95 kJ, 613 次。}
...
\begin{figure}[t]
  ...
  \caption{E3：体验与公平（仅比较机会主义 vs ADCR）。机会主义：反馈 3.80，弱势覆盖 83\%；ADCR：反馈 0.30，弱势覆盖 100\%。}
  \label{fig:e3_feedback_fairness}
\end{figure}

\begin{figure}[t]
  \centering
  \includegraphics[width=0.92\linewidth]{figures/e3_adcr.png}
  \caption{E3：ADCR 的节点能量轨迹。可见多段锯齿式波动与后期能量下沉，印证高频干预带来的能量负担。}
  \label{fig:e3_adcr_energy_traj}
\end{figure}
...
同时，EETOR路径治理在抑制低效传输方面效果显著。从图\ref{fig:e4_path_eff}(a)的路径效率分布可以看出，与无EETOR约束的对照相比，提案机制的路径效率分布明显向右侧（高效区）偏移。量化对比（图\ref{fig:e4_path_eff}(b)）显示，在效率阈值为0.15时，提案机制的低效路径占比仅为9.3%，低于基线的10.6%。这证明了路径治理在减少无效能量损耗、提升单位能量收益方面的直接贡献。
...
% E4 energy trajectory comparison
\paragraph{能量轨迹对比（E4 补充）}
对比基线（DurationAware，自适应时长，见图~\ref{fig:e0_baseline}）与传统 Lyapunov（固定~$\tau$，见图~\ref{fig:e4_lyapunov_energy_traj}）可见：自适应时长使能量演化呈现更规整的阶梯式回补与更缓的整体下沉，节点间带宽更窄、最低能量更高；传统 Lyapunov 则在多个阶段出现更快的群体下挫与更深的低谷，后期下沉更明显。这与图~\ref{fig:e4_feedback_scores_adaptive} 的正向收敛反馈一致，表明时长自适应在既定资源约束下提升了稳态与体验，并减少了无效/过度干预。

\begin{figure}[t]
  \centering
  \includegraphics[width=0.92\linewidth]{figures/e4_lyapunov.png}
  \caption{E4：传统 Lyapunov（固定~$\tau$）的节点能量轨迹，与基线（DurationAware，图~\ref{fig:e0_baseline}）对比可见后期更快下沉与更深低谷。}
  \label{fig:e4_lyapunov_energy_traj}
\end{figure}
...
\begin{table*}[!t]
  ...
  \begin{tabular}{lcccccc}
    ...
    \midrule
    机会主义上报（提案） & 51.90    & 4161.06  & 21.01    & 504  &          & 3.80 \\
    ADCR 点对点上报 & 53.98    & 58530.15 & 140.95   & 613  &          & 0.30 \\
    \bottomrule
  \end{tabular}
\end{table*}
...
\begin{table*}[!t]
  ...
  \begin{tabular}{lcccccccc}
    ...
    传统Lyapunov（固定$\tau$） & 30     & 1149.50 & 575.62  & 573.88 & 50.08    & 4476.58 & 2299 & 97.84    \\
    \bottomrule
  \end{tabular}
\end{table*}
