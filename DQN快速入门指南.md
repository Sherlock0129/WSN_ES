# DQN调度器快速入门指南

## 🚀 5分钟快速开始

### 第一步：训练DQN（推荐）

```bash
# 完整训练（50回合，约3小时，推荐）
python run_dqn_simulation.py --train --episodes 50

# 快速训练（20回合，约1小时，用于快速测试）
python run_dqn_simulation.py --train --episodes 20

# 最小训练（10回合，约30分钟，验证功能）
python run_dqn_simulation.py --train --episodes 10
```

**训练时会看到：**
```
================================================================================
DQN深度强化学习调度器训练
================================================================================

[1] 创建DQN调度器...
  [OK] 动作空间: 10个离散动作（1-10分钟）
  [OK] 初始探索率: 1.0
  [OK] 学习率: 0.001

[2] 开始训练（50回合）...
--------------------------------------------------------------------------------
  回合 10/50: Loss=0.1234, ε=0.6050, Buffer=1000
  回合 20/50: Loss=0.0856, ε=0.3660, Buffer=2000
  回合 30/50: Loss=0.0567, ε=0.2215, Buffer=3000
  回合 40/50: Loss=0.0345, ε=0.1340, Buffer=4000
  回合 50/50: Loss=0.0234, ε=0.0811, Buffer=5000
--------------------------------------------------------------------------------

[3] 训练完成！

[DQN] 模型已保存到: dqn_model.pth
[SUCCESS] 模型已保存到: dqn_model.pth
```

### 第二步：测试DQN

```bash
python run_dqn_simulation.py --test
```

**测试结果示例：**
```
================================================================================
DQN调度器测试
================================================================================

[1] 创建测试网络...
  [OK] 网络节点数: 16

[2] 创建DQN调度器（测试模式）...
  [OK] 模型已加载: dqn_model.pth

[3] 运行仿真（200步）...
--------------------------------------------------------------------------------
[仿真输出...]
--------------------------------------------------------------------------------

[4] 仿真统计:

  传输统计:
  - 总传输次数: 156
  - 平均传输时长: 4.23 分钟
  - 总传输能量: 32900J

  动作使用频率:
     1分钟: ████ 12次 ( 7.7%)
     2分钟: ██████ 18次 (11.5%)
     3分钟: ████████ 24次 (15.4%)
     4分钟: ██████████ 30次 (19.2%)
     5分钟: ████████████ 36次 (23.1%)
     6分钟: ████████ 20次 (12.8%)
     7分钟: ████ 10次 ( 6.4%)
     8-10分钟: ██ 6次 ( 3.9%)

  能量统计:
  - 平均能量: 28456J
  - 能量标准差: 4567J
  - 能量CV: 0.1605

[SUCCESS] 测试完成！
```

### 第三步：运行完整仿真（可选）

```bash
# 运行7天仿真（10080分钟）
python run_dqn_simulation.py --full --steps 10080

# 运行1天仿真（1440分钟）
python run_dqn_simulation.py --full --steps 1440
```

## 📋 完整使用流程

### 方案A：使用run_dqn_simulation.py（推荐）

这是最简单的方式，集成了训练、测试和运行。

```bash
# 1. 训练
python run_dqn_simulation.py --train --episodes 50

# 2. 测试
python run_dqn_simulation.py --test

# 3. 运行完整仿真
python run_dqn_simulation.py --full --steps 10080
```

### 方案B：使用test_dqn_scheduler.py

更详细的训练和测试，包含可视化。

```bash
# 训练+测试（会生成训练曲线图）
python test_dqn_scheduler.py --mode both --episodes 50

# 只训练
python test_dqn_scheduler.py --mode train --episodes 50

# 只测试
python test_dqn_scheduler.py --mode test
```

### 方案C：在自己的代码中使用

```python
from scheduling.dqn_scheduler import DQNScheduler
from acdr.physical_center import NodeInfoManager
from core.energy_simulation import EnergySimulation

# 1. 创建节点信息管理器
nim = NodeInfoManager(
    initial_position=(5.0, 5.0),
    enable_logging=False
)
nim.initialize_node_info(network.nodes, initial_time=0)

# 2. 创建DQN调度器（测试模式）
scheduler = DQNScheduler(
    node_info_manager=nim,
    K=2,
    max_hops=3,
    action_dim=10,
    training_mode=False  # 测试模式
)

# 3. 初始化并加载模型
scheduler.plan(network, 0)  # 触发初始化
scheduler.load_model("dqn_model.pth")

# 4. 运行仿真
simulation = EnergySimulation(
    network=network,
    time_steps=1000,
    scheduler=scheduler,
    enable_energy_sharing=True
)

simulation.simulate()
```

## ⚙️ 参数调整

### 训练参数

| 参数 | 默认值 | 说明 | 调整建议 |
|------|--------|------|---------|
| `--episodes` | 50 | 训练回合数 | 20-100回合 |
| `--steps` | 100 | 每回合步数 | 50-200步 |
| `lr` | 1e-3 | 学习率 | 5e-4 ~ 5e-3 |
| `epsilon_start` | 1.0 | 初始探索率 | 固定1.0 |
| `epsilon_end` | 0.01 | 最终探索率 | 0.01 ~ 0.05 |
| `epsilon_decay` | 0.995 | 探索率衰减 | 0.99 ~ 0.999 |

### 快速训练（测试用）

```python
scheduler = DQNScheduler(
    node_info_manager=nim,
    epsilon_decay=0.99,  # 快速衰减
    buffer_capacity=5000  # 较小缓冲区
)
# 训练10-20回合即可
```

### 精细训练（最优性能）

```python
scheduler = DQNScheduler(
    node_info_manager=nim,
    epsilon_decay=0.998,  # 慢速衰减
    buffer_capacity=20000,  # 大缓冲区
    lr=5e-4  # 较小学习率
)
# 训练100回合以上
```

## 📊 训练监控

### 观察训练进度

**Loss（损失）**：
```
回合 10: Loss=0.1234  ← 初期较大
回合 20: Loss=0.0856  ← 逐渐下降
回合 30: Loss=0.0567  
回合 40: Loss=0.0345  ← 趋于稳定
回合 50: Loss=0.0234  ✓ 收敛
```

**Epsilon（探索率）**：
```
回合 10: ε=0.6050  ← 高探索
回合 20: ε=0.3660  
回合 30: ε=0.2215  
回合 40: ε=0.1340  ← 逐渐减少
回合 50: ε=0.0811  ← 低探索，主要利用
```

**Buffer（经验缓冲区）**：
```
回合 10: Buffer=1000  ← 逐渐填充
回合 20: Buffer=2000  
回合 30: Buffer=3000  
...
Buffer满（10000）后开始覆盖旧经验
```

### 判断训练质量

**好的训练**：
- ✅ Loss从0.1+下降到0.05以下
- ✅ Epsilon从1.0衰减到0.1以下
- ✅ Buffer逐渐增长到数千
- ✅ 后期Loss趋于稳定

**需要继续训练**：
- ⚠️ Loss仍在0.1以上
- ⚠️ Loss剧烈震荡
- ⚠️ Epsilon还很高（>0.3）

## 🔧 故障排除

### 问题1：训练Loss不下降

**原因**: 学习率过大或过小

**解决**:
```python
scheduler = DQNScheduler(
    lr=5e-4,  # 降低学习率
    ...
)
```

### 问题2：训练不稳定，Loss震荡

**原因**: 探索率衰减过快或网络更新过于频繁

**解决**:
```python
scheduler = DQNScheduler(
    epsilon_decay=0.998,  # 慢速衰减
    tau=0.001,  # 更小的软更新系数
    ...
)
```

### 问题3：模型文件不存在

**错误信息**:
```
[ERROR] 模型文件不存在: dqn_model.pth
请先运行训练：python run_dqn_simulation.py --train
```

**解决**: 先训练模型
```bash
python run_dqn_simulation.py --train --episodes 20
```

### 问题4：内存不足

**解决**: 减小网络规模和缓冲区
```python
config.network_config.num_nodes = 10  # 减少节点数
scheduler = DQNScheduler(
    buffer_capacity=5000,  # 减小缓冲区
    ...
)
```

### 问题5：训练太慢

**解决**: 
1. 减少回合数和步数
```bash
python run_dqn_simulation.py --train --episodes 20
```

2. 使用GPU（如果有）
```python
# PyTorch会自动使用GPU
# 检查：torch.cuda.is_available()
```

3. 减小网络规模
```python
config.network_config.num_nodes = 10
config.simulation_config.time_steps = 50
```

## 📈 性能提升建议

### 训练策略

**阶段1：快速探索（前20回合）**
```python
epsilon_decay = 0.99  # 快速衰减
episodes = 20
```

**阶段2：精细优化（后30回合）**
```python
epsilon_decay = 0.998  # 慢速衰减
lr = 5e-4  # 降低学习率
episodes = 30
```

### 超参数网格搜索

```bash
# 测试不同学习率
for lr in 5e-4 1e-3 5e-3; do
    python run_dqn_simulation.py --train --episodes 30
done

# 测试不同探索率衰减
for decay in 0.99 0.995 0.998; do
    python run_dqn_simulation.py --train --episodes 30
done
```

## 📁 文件说明

| 文件 | 说明 |
|------|------|
| `run_dqn_simulation.py` | **快速启动脚本**（推荐） |
| `test_dqn_scheduler.py` | 详细训练测试脚本（含可视化） |
| `dqn_model.pth` | 训练好的模型文件 |
| `dqn_training_curves.png` | 训练曲线图（由test_dqn生成） |
| `src/scheduling/dqn_scheduler.py` | DQN调度器实现 |
| `DQN离散动作调度器说明.md` | 详细技术文档 |

## 💡 最佳实践

### 1. 训练流程

```bash
# Step 1: 快速验证（30分钟）
python run_dqn_simulation.py --train --episodes 10

# Step 2: 测试效果
python run_dqn_simulation.py --test

# Step 3: 如果效果好，完整训练（3小时）
python run_dqn_simulation.py --train --episodes 50

# Step 4: 运行长期仿真
python run_dqn_simulation.py --full --steps 10080
```

### 2. 模型版本管理

```bash
# 保存不同版本
python run_dqn_simulation.py --train --episodes 20 --model dqn_v1.pth
python run_dqn_simulation.py --train --episodes 50 --model dqn_v2.pth
python run_dqn_simulation.py --train --episodes 100 --model dqn_v3.pth

# 测试不同版本
python run_dqn_simulation.py --test --model dqn_v1.pth
python run_dqn_simulation.py --test --model dqn_v2.pth
```

### 3. 对比测试

```python
# 测试DQN vs Lyapunov
from scheduling.schedulers import LyapunovScheduler

# 运行两次仿真，对比结果
# 1. DQN
# 2. Lyapunov
```

## 🎯 推荐配置

### 快速测试配置

```bash
python run_dqn_simulation.py --train --episodes 10
```
- 时间：~30分钟
- 用途：验证功能
- 性能：中等

### 标准配置（推荐）⭐

```bash
python run_dqn_simulation.py --train --episodes 50
```
- 时间：~3小时
- 用途：实际使用
- 性能：优秀

### 高性能配置

```bash
python run_dqn_simulation.py --train --episodes 100
```
- 时间：~6小时
- 用途：追求最优
- 性能：最佳

## 📞 获取帮助

```bash
# 查看帮助信息
python run_dqn_simulation.py --help

# 查看详细文档
# 阅读：DQN离散动作调度器说明.md
```

## ✅ 检查清单

训练前：
- [ ] 已安装PyTorch
- [ ] 已安装NumPy、Matplotlib
- [ ] 确认有足够磁盘空间（~500MB）

训练中：
- [ ] 观察Loss是否下降
- [ ] 观察Epsilon是否衰减
- [ ] 确认Buffer在增长

训练后：
- [ ] 生成了dqn_model.pth文件
- [ ] 运行测试验证性能
- [ ] 查看动作分布是否合理

---

**祝您使用愉快！** 🎉

如有问题，请查阅：
- `DQN离散动作调度器说明.md` - 详细技术文档
- `深度学习调度器对比.md` - 方法对比

